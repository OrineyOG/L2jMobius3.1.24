...
public abstract class Creature extends WorldObject implements ISkillsHolder, IDeletable
{
	...
	private Attack generateAttackTargetData(Creature target, Weapon weapon, WeaponType weaponType)
	{
		...
		// Initialize attackCountMax for chaining
		int attackCountMax = (int) _stat.getValue(Stat.ATTACK_COUNT_MAX, 1);
		int initialDamage = 0;
		int numberOfChainedHits = 0;
		double damageReductionFactor = 0.5; // 50% reduction for each chain
		double chainChance = _stat.getValue(Stat.CHANCE_TO_CHAIN, 0);
		
		// Print chain chance only for player attacks
		if (this instanceof Player)
		{
			System.out.println("Chance to Chain: " + chainChance);
		}
		
		Set<Creature> attackedTargets = new HashSet<>();
		// Main target hit
		Hit hit = generateHit(target, weapon, shotConsumed, isDual, initialDamage);
		attack.addHit(hit);
		initialDamage = hit.getDamage();
		shotConsumed = hit.isShotUsed();
		attackedTargets.add(target);
		
		// Check for chain attack chance
		if ((Math.random() * 100) < chainChance)
		{
			// Chaining logic
			if (attackCountMax > 1)
			{
				for (int i = 1; i < attackCountMax; i++)
				{
					// Find the nearest target
					Creature nearestTarget = findNearestTarget(target, attackedTargets);
					...
					// Add the nearest target to the attackedTargets set
					attackedTargets.add(nearestTarget);
					
					// Calculate reduced damage
					initialDamage *= damageReductionFactor;
					
					// Generate hit for the new target with reduced damage
					Hit chainHit = generateHit(nearestTarget, weapon, shotConsumed, false, initialDamage);
					
					// Add the hit to the attack
					attack.addHit(chainHit);
					
					// Increment the number of chained hits
					numberOfChainedHits++;
					
					// Update shotConsumed
					shotConsumed = chainHit.isShotUsed();
				}
			}
		}
		else
		{
			...
			if (isDual)
			{
				hit = generateHit(target, weapon, shotConsumed, isDual, initialDamage);
				attack.addHit(hit);
				shotConsumed = hit.isShotUsed();
			}
			...
			if ((attackCountMax > 1) && (_stat.getValue(Stat.PHYSICAL_POLEARM_TARGET_SINGLE, 0) <= 0))
			{
				...
				for (Creature obj : World.getInstance().getVisibleObjectsInRange(this, Creature.class, maxRadius))
				{
					...
					hit = generateHit(obj, weapon, shotConsumed, false, initialDamage);
					attack.addHit(hit);
					shotConsumed = hit.isShotUsed();
					if (--attackCountMax <= 0)
					{
		...
		// Notify the player about the total number of chained hits, if any
		if ((this instanceof Player) && (numberOfChainedHits > 0))
		{
			((Player) this).sendMessage("Your arrow chained to " + numberOfChainedHits + " additional target(s)!");
		}
		
		return attack;
	}
	
	private Creature findNearestTarget(Creature currentTarget, Set<Creature> attackedTargets)
	{
		double minDistance = Double.MAX_VALUE;
		Creature nearestTarget = null;
		final int maxRadius = 400; // The actual radius
		
		for (Creature obj : World.getInstance().getVisibleObjectsInRange(this, Creature.class, maxRadius))
		{
			// Skip the current target and any already attacked targets
			if ((obj == currentTarget) || attackedTargets.contains(obj))
			{
				continue;
			}
			
			// Skip dead or fake dead targets
			if (obj.isAlikeDead())
			{
				continue;
			}
			
			// Check if the target is auto-attackable
			if (!obj.isAutoAttackable(this))
			{
				continue;
			}
			
			// Check if the object is within the radius of the current target
			if (!currentTarget.isInsideRadius2D(obj, maxRadius))
			{
				continue;
			}
			
			// Calculate distance to the current target
			double distance = currentTarget.calculateDistance2D(obj);
			
			// Update nearest target if this one is closer
	...
	private Hit generateHit(Creature target, Weapon weapon, boolean shotConsumedValue, boolean halfDamage, int initialDamage)
	{
