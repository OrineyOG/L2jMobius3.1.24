...
public class CreatureStat
{
	...
	private final Map<Stat, Double> _statsAddMul = new EnumMap<>(Stat.class);
	...
	private final Deque<StatHolder> _additionalAddMul = new ConcurrentLinkedDeque<>();
	...
	{
		double modifier = getValue(Stat.BOW_ATTACK_REUSE, 100); // Default is 100 for no change
		return modifier / 100; // Converts the value to a multiplier
	}
	...
	/**
	 * Merges the stat's value with the values within the map of addmuls
	 * @param stat
	 * @param value
	 */
	public void mergeAddMul(Stat stat, Double value)
	{
		_statsAddMul.merge(stat, value, stat::functionAddMul);
	}
	...
	/**
	 * @param stat
	 * @return the addmul value
	 */
	public double getAddMul(Stat stat)
	{
		return getAddMul(stat, 1d);
	}
	 ...
	 * @return the addmul value
	 */
	public double getAddMul(Stat stat, double defaultValue)
	{
		_lock.readLock().lock();
		try
		{
			final Double val = _statsAddMul.get(stat);
	...
	protected void resetStats()
	{
		_statsAdd.clear();
		_statsMul.clear();
		_statsAddMul.clear(); // Clear the AddMul stats.
		_vampiricSum = 0;
		_mpVampiricSum = 0;
		...
		for (Stat stat : Stat.values())
		{
			...
			// Initialize AddMul values.
			final Double resetAddMulValue = stat.getResetAddMulValue();
			if (resetAddMulValue.doubleValue() != 0)
			{
				_statsAddMul.put(stat, resetAddMulValue);
			}
		}
	}
	...
	public void recalculateStats(boolean broadcast)
	{
		...
		final Map<Stat, Double> addMuls = !broadcast ? Collections.emptyMap() : new EnumMap<>(_statsAddMul);
		...
		try
		{
			...
			for (StatHolder holder : _additionalAddMul)
			{
				if (holder.verifyCondition(_creature))
				{
					mergeAddMul(holder.getStat(), holder.getValue());
				}
			}
		...
		if (broadcast)
		{
			...
			for (Stat stat : Stat.values())
			{
				if (!_statsAdd.getOrDefault(stat, stat.getResetAddValue()).equals(adds.getOrDefault(stat, stat.getResetAddValue())) || !_statsMul.getOrDefault(stat, stat.getResetMulValue()).equals(muls.getOrDefault(stat, stat.getResetMulValue())) || !_statsAddMul.getOrDefault(stat, stat.getResetAddMulValue()).equals(addMuls.getOrDefault(stat, stat.getResetAddMulValue())))
				{
					changed.add(stat);
				}
			}
	...
	/**
	 * Adds static addative multiplier to the 'addmul' map of the stat everytime recalculation happens
	 * @param stat
	 * @param value
	 * @param condition
	 * @return
	 */
	public boolean addMulAdditionalStat(Stat stat, double value, BiPredicate<Creature, StatHolder> condition)
	{
		return _additionalAddMul.add(new StatHolder(stat, value, condition));
	}
	 ...
	 * @return {@code true}
	 */
	public boolean addMulAdditionalStat(Stat stat, double value)
	{
		return _additionalAddMul.add(new StatHolder(stat, value));
	}
	
	/**
	 * @param stat
	 * @param value
	 * @return {@code true} if 'addmul' was removed, {@code false} in case there wasn't such stat and value
	 */
	public boolean removeAddMulAdditionalStat(Stat stat, double value)
	{
		final Iterator<StatHolder> it = _additionalAddMul.iterator();
		while (it.hasNext())
		{
			final StatHolder holder = it.next();
			if ((holder.getStat() == stat) && (holder.getValue() == value))
