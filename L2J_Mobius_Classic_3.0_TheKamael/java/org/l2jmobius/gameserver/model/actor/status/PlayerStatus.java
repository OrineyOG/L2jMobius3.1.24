...
public class PlayerStatus extends PlayableStatus
{
	...
	public void reduceHp(double value, Creature attacker, Skill skill, boolean awake, boolean isDOT, boolean isHPConsumption, boolean ignoreCP)
	{
		...
		if ((attacker != null) && (attacker != getActiveChar()))
		{
			...
			double transferToManaPercent = getActiveChar().getStat().getValue(Stat.TRANSFER_DAMAGE_TO_MANA, 0);
			if ((transferToManaPercent > 0) && ((getActiveChar().getCurrentMp() / getActiveChar().getMaxMp()) > 0.15))
			{
				int mpTransDam = (int) (amount * (transferToManaPercent / 100.0));
				if (mpTransDam > getActiveChar().getCurrentMp())
				{
					mpTransDam = (int) getActiveChar().getCurrentMp();
				}
				getActiveChar().reduceCurrentMp(mpTransDam);
				amount -= mpTransDam;
				fullValue = (int) amount;
			}
