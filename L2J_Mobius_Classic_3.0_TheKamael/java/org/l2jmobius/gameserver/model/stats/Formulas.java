...
public class Formulas
{
	...
	public static double calcMagicDam(Creature attacker, Creature target, Skill skill, double mAtk, double power, double mDef, boolean sps, boolean bss, boolean mcrit)
	{
	    ...
	    // Add True Damage as a flat value
	    double trueDamage = attacker.getStat().getAdd(Stat.TRUE_DAMAGE);
	    damage += trueDamage;
	...
	{
		// Placeholder for armor speed bonus
		final double armorBonus = 1; // TODO: Implement EquipedArmorSpeedByCrystal
		
		// Bonus based on Dexterity
		final double dexBonus = BaseStat.DEX.calcBonus(creature);
		
		// Base attack speed of the weapon, adjusted by armor bonus
		final double weaponAttackSpeed = Stat.weaponBaseValue(creature, Stat.PHYSICAL_ATTACK_SPEED) / armorBonus;
		
		// Multiplier, additive, and additional multiplier for physical attack speed
		...
		final double attackSpeedAddBonus = creature.getStat().getAddMul(Stat.PHYSICAL_ATTACK_SPEED);
		
		// Combined multiplier, similar to the one in defaultValue()
		final double combinedMul = (attackSpeedPerBonus + attackSpeedAddBonus) - 1;
		
		// Final calculation, retaining the division by 333
		return (dexBonus * (weaponAttackSpeed / 333) * combinedMul) + ((attackSpeedDiffBonus / 333) * attackSpeedPerBonus);
	}
	...
	public static double calcMAtkSpdMultiplier(Creature creature)
	{
		...
		// Multiplier, additive, and additional multiplier for magic attack speed
		...
		final double castingSpeedAddBonus = creature.getStat().getAddMul(Stat.MAGIC_ATTACK_SPEED);
		
		// Combined multiplier, similar to the one in previous methods
		final double combinedMul = (castingSpeedPerBonus + castingSpeedAddBonus) - 1;
		
		// Final calculation, retaining the division by 333
		return (witBonus * (1 / armorBonus) * combinedMul) + ((castingSpeedDiffBonus / 333) * castingSpeedPerBonus);
	}
	...
	public static List<BuffInfo> calcCancelStealEffects(Creature creature, Creature target, Skill skill, DispelSlotType slot, int rate, int max)
	{
		...
		switch (slot)
		{
			...
			case SELFBUFF:
			{
				// Logic to prioritize self-buffs
				final List<BuffInfo> selfBuffs = target.getEffectList().getSelfBuffs();
				for (BuffInfo info : selfBuffs)
				{
					if (info.getSkill().canBeStolen() && (Rnd.get(100) <= rate))
					{
						canceled.add(info);
			...
			case BUFF:
			{
				...
				// Initialize a list to hold all eligible buffs
				final List<BuffInfo> allEligibleBuffs = new ArrayList<>();
				
				// Add dances
				final List<BuffInfo> dances = target.getEffectList().getDances();
				for (BuffInfo info : dances)
				{
					if (info.getSkill().canBeStolen() && calcCancelSuccess(info, cancelMagicLvl, rate, skill, target))
					{
						allEligibleBuffs.add(info);
					}
				}
				...
				System.out.println("Pre-shuffled list: " + allEligibleBuffs);
				// use RNGesus
				Collections.shuffle(allEligibleBuffs);
				
				// Add the first buffs to be canceled
				for (int i = 0; i < Math.min(max, allEligibleBuffs.size()); i++)
				{
					canceled.add(allEligibleBuffs.get(i));
				}
	...
	public static int calculateReuseTime(Creature creature, Weapon weapon)
	{
		...
		// Only ranged weapons should continue.
		if (!weaponType.isRanged())
		{
			return 0;
		}
		
		int reuse = 900000 / creature.getStat().getPAtkSpd();
		
		// Apply the weapon reuse modifier for all ranged weapons.
