...
public class PAttackSpeedFinalizer implements IStatFunction
{
	...
	{
		// Ensure the multiplier is at least 0.7
		final double mul = Math.max(creature.getStat().getMul(stat), 0.7);
		
		// Get the additive value for the stat
		final double add = creature.getStat().getAdd(stat);
		
		// Get the additional multiplier for the stat
		final double addmul = creature.getStat().getAddMul(stat);
		
		// Calculate the combined multiplier
		final double combinedMul = (mul + addmul) - 1;
		
		// Get the move type value for the creature
		final double moveTypeValue = creature.getStat().getMoveTypeValue(stat, creature.getMoveType());
		
		// Final calculation
		return (baseValue * combinedMul) + add + moveTypeValue;
	}
}
