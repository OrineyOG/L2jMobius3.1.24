...
public class MaxHpFinalizer implements IStatFunction
{
	...
	private static double defaultValue(Creature creature, Stat stat, double baseValue)
	{
		...
		double addmul = creature.getStat().getAddMul(stat);
		
		// Initialize maxHP bonus from items and EnchantItemBonus
		double maxHPbonusitems = 0.0;
		double EnchantItemBonus = 0.0;
		...
		if (inv != null)
		{
			...
			{
				maxHPbonusitems += item.getTemplate().getStats(stat, 0);
				...
				if (item.isArmor() && item.isEnchanted())
				{
					...
					{
						EnchantItemBonus += EnchantItemHPBonusData.getInstance().getHPBonus(item);
					}
				}
			}
		}
		...
		// New maxHp formula
		double maxHp = (((baseValue * addmul) + add + maxHPbonusitems + EnchantItemBonus) * mul) + creature.getStat().getMoveTypeValue(stat, creature.getMoveType());
		...
		if (isPlayer)
		{
			...
			final double hpLimit = (Config.MAX_HP * mul) + add;
