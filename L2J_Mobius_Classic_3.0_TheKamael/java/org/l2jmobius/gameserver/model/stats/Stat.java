...
public enum Stat
{
	...
	MAGIC_ATTACK_BY_PHYSICAL_ATTACK("mAtkByPAtk", Stat::defaultValue, MathUtil::add, MathUtil::mul, MathUtil::addMul, 0, 0, 0),
	...
	BOW_ATTACK_REUSE("BowAttackReuse"), // Bows Hits Reuse Rate
	...
	CRITICAL_RATE("rCrit", new PCriticalRateFinalizer(), MathUtil::add, MathUtil::add, MathUtil::addMul, 0, 0, 0),
	...
	MAGIC_CRITICAL_RATE_BY_CRITICAL_RATE("mCritRateByRCrit", Stat::defaultValue, MathUtil::add, MathUtil::mul, MathUtil::addMul, 0, 0, 0),
	BLOW_RATE("blowRate"),
	...
	TRANSFER_DAMAGE_TO_MANA("transDamToMP"),
	...
	private final DoubleBinaryOperator _addmulFunction;
	private final Double _resetAddValue;
	private final Double _resetMulValue;
	private final Double _resetAddMulValue;
	...
	{
		this(xmlString, Stat::defaultValue, MathUtil::add, MathUtil::mul, MathUtil::addMul, 0, 1, 1);
	}
	
	Stat(String xmlString, IStatFunction valueFinalizer)
	{
		this(xmlString, valueFinalizer, MathUtil::add, MathUtil::mul, MathUtil::addMul, 0, 1, 1);
	}
	
	Stat(String xmlString, IStatFunction valueFinalizer, DoubleBinaryOperator addFunction, DoubleBinaryOperator mulFunction, DoubleBinaryOperator addMulFunction, double resetAddValue, double resetMulValue, double resetAddMulValue)
	{
		_value = xmlString;
		_valueFinalizer = valueFinalizer;
		_addFunction = addFunction;
		_mulFunction = mulFunction;
		_addmulFunction = addMulFunction;
		_resetAddValue = resetAddValue;
		_resetMulValue = resetMulValue;
		_resetAddMulValue = resetAddMulValue;
	}
	...
	public double functionAddMul(double oldValue, double value)
	{
		return _addmulFunction.applyAsDouble(oldValue, value);
	}
	...
	public Double getResetAddMulValue()
	{
		return _resetAddMulValue;
	}
	...
	public static double defaultValue(Creature creature, OptionalDouble base, Stat stat)
	{
		...
		final double addmul = creature.getStat().getAddMul(stat);
		return base.isPresent() ? (((base.getAsDouble() * addmul) + add) * mul) + creature.getStat().getMoveTypeValue(stat, creature.getMoveType()) : mul * (add + creature.getStat().getMoveTypeValue(stat, creature.getMoveType()));
	}
	...
	public static double defaultValue(Creature creature, Stat stat, double baseValue)
	{
		...
		final double add = creature.getStat().getAdd(stat);
		final double addmul = creature.getStat().getAddMul(stat);
		return (((baseValue * addmul) + add) * mul) + creature.getStat().getMoveTypeValue(stat, creature.getMoveType());
	}
